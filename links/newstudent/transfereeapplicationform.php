...
header('Location: transfereelogin.php');
exit;
                            ...
                            if(isset($_POST['submit']))
                          {
                              ...
                              $gcon = mysql_real_escape_string(htmlspecialchars(trim($_POST['guardiancontact'])));
                                ...
                                $x++; 
                                        ...
                                        VALUES ('$x','$db_email','$year','$seeking','$applied','$db_status','$Sname','$Fname','$Mname','PENDING')");
                                        mysql_query($forms);
                                        ...
                                        header('url=transfereeapplicationform.php?Success!');
                        }
                          ...
                          $email=$_POST['email'];
                            ...
                            $remail = mysql_query("SELECT email FROM tbl_studentregistration WHERE email='$email'");
                                                                $checkemail = mysql_num_rows($remail);
                                                                      if ($checkemail != 0){
                                                                                header("Location: transfereeapplicationform.php?EmailAlreadyRegistered=error!");
                                                                                
                                                                                            }else{
                                                                                                mysql_query("UPDATE tbl_studentregistration set email='$email' where username='$username'");
                                                                                                header("Location: transfereeapplicationform.php?updatesuccess!");
                                                                                                  }
                        }
...
disp_setting+="scrollbars=yes,width=650, height=600, left=100, top=25";
   ...
   docprint.document.write('<br><center><B>APPLICATION FOR ADMISSION (TRANSFEREE)</B></center>');
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      ...
      <ul class="nav navbar-nav navbar-right">
        <li> <a href="transfereelogout.php"><i class="glyphicon glyphicon-log-out"></i> Logout</a></li>
      </ul>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
</nav>  
...
$message= "Confirm your email
  Click the link below to verify your account 
    http://www.sims-mks.com/transfereeapplicationform.php?emailconfirmed=$email&code=$confirmed
";  
